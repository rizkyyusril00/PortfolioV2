...
export default function BtnSosmed (){
    ...
    return(
        <>
            {sosmeds.map((sosmed) => (
                <a href={sosmed.url} aria-label="icon-sosmed" target="_blank" key={sosmed.id} className="w-8 h-8 md:w-6 md:h-6 slg:w-9 slg:h-9 lg:w-12 lg:h-12 bg-[#00010D] rounded-full flex items-center justify-center relative group">
                    <i className={sosmed.icon + " text-[16px] md:text-[14px] slg:text-[18px] lg:text-[22px] text-primary relative z-20"}></i>

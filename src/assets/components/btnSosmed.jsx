    ...
    return (
        <>
            {sosmeds.map((sosmed) => (
                <a
                href={sosmed.url}
                aria-label="icon-sosmed"
                target="_blank"
                key={sosmed.id}
                className="w-8 h-8 md:w-6 md:h-6 slg:w-9 slg:h-9 lg:w-12 lg:h-12 bg-[#00010D] rounded-full flex items-center justify-center relative group"
                >
                    <i
                        className={
                        sosmed.icon +
                        " text-[16px] md:text-[14px] slg:text-[18px] lg:text-[22px] text-primary relative z-20"
                        }
                    ></i>
                    <div
                        className={`${sosmed.hoverStyle} absolute -top-1 w-0 h-0 scale-0 opacity-0 rounded-full group-hover:w-6 group-hover:h-6 slg:group-hover:w-9 slg:group-hover:h-9 lg:group-hover:w-12 lg:group-hover:h-12 transition-all duration-500 ease-in-out group-hover:top-0 group-hover:opacity-100 group-hover:scale-100`}
                    ></div>
                </a>
            ))}
        </>
